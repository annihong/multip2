...
simulations <- grep("ds$", simulations, value = TRUE)
...
calc_stats_file <- function(simulation_file){
    ...
    file_path <- paste0(path, "sim_res/new/", simulation_file)
    #print(simulation_file)
...
save(res, file=paste0(path, "analysis_res/sim_analysis_df_full_", length(simulations), MODE, ".RData"))
load(paste0(path, "analysis_res/sim_analysis_df_full_", MODE, ".RData"))
saveRDS(list(params=sampled_params_list, draws=posterior_draws_list), file=paste0(path, "analysis_res/final_res_1000.rds"))
# PS_mu_prior <- read.csv(file = "/home/annihong/projects/simres/analysis_res/PS_mu_prior.csv")[,c(2,3)]
# v_PS_mu <- (sd(unlist(PS_mu_prior)))^2
...
# rank_helper <- function(to_rank, vector){
#     #subsample <- vector[-1000]
#     subsample <- sample(vector, 999)
#     # print(length(subsample))
#     rank <- rank(c(to_rank, subsample))[1]
#     return(rank)
# }
...
# saveRDS(list(params=sampled_params_list, draws=posterior_draws_list), file=paste0(path, "analysis_res/final1000.rds"))
...
hist <- ggplot(tidyr::gather(rank_df)) + geom_histogram(aes(x=value/101, color = key), size = 1, breaks=seq(0,1,0.05)) +    
    ...
    ggsave(paste0(path, "plots/new", MODE, "_2.png"), plot=hist, width = 8, height = 2)
